...
pizzaJson.map((item, index)=>{
    ...
    // preencher o indice de cada pizza:
    pizzaItem.setAttribute('data-key', index);
    //preencher a imagem da pizza:
    pizzaItem.querySelector('.pizza-item--img img').src= item.img;
    //preencher o preço da pizza:
    pizzaItem.querySelector('.pizza-item--price').innerHTML = `R$ ${item.price.toFixed(2)}`;
    //preencher o nome da pizza:
    pizzaItem.querySelector('.pizza-item--name').innerHTML = item.name;
    //preencher a descrição da pizza:
    pizzaItem.querySelector('.pizza-item--desc').innerHTML = item.description;

    //MODAL
    //Adicionar um evento de click a tag <a>:
    pizzaItem.querySelector('a').addEventListener('click', (e)=>{
        e.preventDefault();
        let key = e.target.closest('.pizza-item').getAttribute('data-key');
        modalQt = 1;
        modalKey = key;
        c('.pizzaBig img').src = innerHTML = pizzaJson[key].img;
